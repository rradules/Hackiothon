...
public class GaugeView extends View {
    ...
    private void readAttrs(final Context context, final AttributeSet attrs, final int defStyle) {
        ...
        CharSequence[] x = a.getTextArray(R.styleable.GaugeView_rangeValues);
        mRangeValues = new float[x.length];
        for(int i = 0; i < x.length; i++)
            mRangeValues[i] = Float.parseFloat(x[i].toString());
    ...
    private void drawScale(final Canvas canvas) {
        ...
        for (int i = 0; i < totalTicks; i++) {
            ...
            float div = (mScaleEndValue - mScaleStartAngle) / (float) mDivisions;
    ...
    public void setTargetValue(float value) {
        ...
        if(this.mRangeValues[0] < 0) {
            float zeroBase = Math.abs(this.mRangeValues[0]);
